...
&emsp;&emsp;每当开始一个事务时，当前版本号是从innodb事务系统申请的，并且按照申请顺序严格递增。当前版本等于全局系统版本，即currentTransactionId=sysTransactionId  
- 查询：currentTransactionId>=startTransactionId && (endTransactionId == null \|\| currentTransactionId<endTransactionId)。查询时当前获取到的事务的版本号(系统版本号)需要大于或等于创建版本号，同时当前事务的版本号需要小于删除的版本号。也即未被后面的事务修改，也还存活。  
- 插入：设置 startTransactionId = currentTransactionId，endTransactionId为空
- 删除：设置 endTransactionId = currentTransactionId
- 更新：设置 startTransactionId = currentTransactionId &&  endTransactionId = currentTransactionId

####  MVCC示例解析
&emsp;&emsp;在实现上，innodb为每个事务构建一个数组，用来保存这个事务启动瞬间，当前正在活跃的所有事务ID （活跃指的是启动了但还未提交）
...
&emsp;&emsp;根据事务对记录的操作时间点的先后顺序，构成下面的一个TransactionId事务操作链表：  
...
g.因此事务A [99,100]查询数据时候，由于101,102对它不可见，所以事务A查询结果为1，又由于update为当前读(使用时读取，另外两个已经提交)，所以事务B查询结果为3。    
...
####  临界锁 next-key
&emsp;&emsp;通过上面MVCC分析可知，可重复读和提交读是矛盾的。在同一个事务里，如果保证了可重复读，就会看不到其他事务的提交读取的总是快照旧数据，违背了提交读；如果保证了提交读，就会导致前后两次读到的结果不一致，违背了可重复读。    
&emsp;&emsp;为左开右闭，左间隙与行锁组合起来用就叫做Next-Key
Lock，间隙锁主要创建在联合索引或者主键索引中，其是将查找到的主键前后节点之间的插入关闭。  
...
&emsp;&emsp;insert在做插入时，其加锁过程为先在插入间隙上获取插入意向锁，插入数据后再获取插入行上的排它锁。插入意向锁与gap
lock和 Next-key lock冲突的，需要等待这两种锁释放。

-----

#### 操作日志
&emsp;&emsp;redo log 记录事务修改后的数据, undo log 记录事务前的原始数据。    
...
5. 在事务操作时，检测到存在排它锁，进入事务挂起等待阶段，等前面的事务提交后才继续执行。<font color="blue">RR下为了防止对同一条数据变更出现可重复读并发变更，需要再变更前使用select ... for update进行锁定？？？还是在提交时检测DB_TRX_ID字段回滚？？？一锁二查三变更</font>
...
3. 刷redo日志：通过redo日志落盘操作，保证了即使修改的数据页没有即使更新到磁盘，只要日志是完成了，就能保证数据库的完整性和一致性；
