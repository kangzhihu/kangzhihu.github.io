...
&emsp;&emsp;MVCC在使用READ COMMITTD、REPEATABLE READ这两种隔离级别的事务在执行普通的SEELCT操作时访问记录的版本链的过程，这样子可以使不同事务的读-写、写-读操作并发执行，从而提升系统性能。      
...
&emsp;&emsp;☞每次读取数据前都生成一个ReadView,这样当前事务持有的ReadView中一直包含最新的事务id，所以一直可以获得当前最新的数据。-- 可以从当前m_ids列表中剔除已提交的事务id，若这个事务id比当前事务id小，那么就能读取到了。  
