...
&emsp;&emsp;<font color="red">业务先开启事务做资源校验(库存是足够、中间件等是否可用)&预先锁定字段(冻结库存等)，从而避免2pc中其他事务直接开始操作业务而某一个失败后整体做无用功一直阻塞？CanCommit存在原因是比比PreCommit执行完后回滚成本低能事务快失败？</font> 
...
**<font color = 'red'>从整体上来说，TCC不存在一个全局的事务锁。其锁粒度是每个远程服务维度，通过引入中间状态使得每次远程操作后，当前的远程操作可以直接进行事务提交。当远程事务提交后，对服务系统来说，其相关资源将被释放，相对而言大大的提高了系统的吞吐量。</font>**
